...
function renderDashboard() {
    ...
    <div class="dashboard-grid">
        <div class="card dashboard-wide"><div class="card-header"><h3 data-i18n="dash_servers">${t('dash_servers')}</h3><button class="btn btn-primary btn-sm" onclick="loadPage('setup')" data-i18n="dash_addserver">${t('dash_addserver')}</button></div><div class="card-body">
            <div class="dashboard-summary">
                <div class="summary-stat"><span class="summary-label">${t('dash_online')}</span><strong id="summary-online">0</strong></div>
                <div class="summary-stat"><span class="summary-label">${t('dash_offline')}</span><strong id="summary-offline">0</strong></div>
                <div class="summary-stat"><span class="summary-label">${t('dash_uptime')}</span><strong id="summary-uptime">00:00:00</strong></div>
            </div>
            <div id="serverOverview" class="server-overview-grid"></div>
        </div></div>
    </div>`;
    loadServerOverview();
}
...
function loadServerOverview() {
    get('/api/servers/status', function(d) {
        ...
        if (!el || !d.success || !Array.isArray(d.servers)) return;
        serverStatusById = {};
        var onlineCount = 0;
        var offlineCount = 0;
        el.innerHTML = d.servers.map(function(server) {
            serverStatusById[server.id] = server;
            var active = server.id === d.activeServerId;
            var worldRunning = !!(server.worldserver && server.worldserver.running);
            var bnetRunning = !!(server.bnetserver && server.bnetserver.running);
            var overallRunning = worldRunning || bnetRunning;
            if (overallRunning) onlineCount++; else offlineCount++;
            return '<div class="server-overview-item ' + (active ? 'active' : '') + '">' +
                '<div class="server-overview-head"><span class="server-overview-dot ' + (overallRunning ? 'online' : '') + '"></span><span class="server-overview-tag">' + (overallRunning ? t('dash_online') : t('dash_offline')) + '</span></div>' +
                '<span class="server-overview-main"><strong>' + esc(server.name) + '</strong><small>' + (server.mode === 'ssh' ? 'SSH: ' + esc(server.host || 'unknown') : server.mode) + '</small></span>' +
                '<div class="server-metrics"><span><i class="server-metric-dot ' + (worldRunning ? 'online' : '') + '"></i>World: ' + (worldRunning ? t('dash_online') : t('dash_offline')) + '</span><span><i class="server-metric-dot ' + (bnetRunning ? 'online' : '') + '"></i>BNet: ' + (bnetRunning ? t('dash_online') : t('dash_offline')) + '</span></div>' +
                '<span class="server-overview-actions">' +
                    '<button class="btn btn-success btn-sm" ' + (worldRunning ? 'disabled' : '') + ' onclick="serverTileAction(event,\'' + esc(server.id) + '\',\'start\',\'worldserver\')" title="' + t('dash_wstart') + '">W+</button>' +
                    '<button class="btn btn-danger btn-sm" ' + (worldRunning ? '' : 'disabled') + ' onclick="serverTileAction(event,\'' + esc(server.id) + '\',\'stop\',\'worldserver\')" title="' + t('dash_wstop') + '">W-</button>' +
                    '<button class="btn btn-success btn-sm" ' + (bnetRunning ? 'disabled' : '') + ' onclick="serverTileAction(event,\'' + esc(server.id) + '\',\'start\',\'bnetserver\')" title="' + t('dash_astart') + '">B+</button>' +
                    '<button class="btn btn-danger btn-sm" ' + (bnetRunning ? '' : 'disabled') + ' onclick="serverTileAction(event,\'' + esc(server.id) + '\',\'stop\',\'bnetserver\')" title="' + t('dash_astop') + '">B-</button>' +
                    '<button class="btn btn-secondary btn-sm" onclick="serverTileOpen(event,\'' + esc(server.id) + '\',\'config\')" title="' + t('cfg_title') + '">CFG</button>' +
                    '<button class="btn btn-secondary btn-sm" onclick="serverTileOpen(event,\'' + esc(server.id) + '\',\'console\')" title="' + t('dash_console') + '">&gt;_</button>' +
                '</span></div>';
        }).join('');
        var onlineEl = document.getElementById('summary-online');
        var offlineEl = document.getElementById('summary-offline');
        var uptimeEl = document.getElementById('summary-uptime');
        if (onlineEl) onlineEl.textContent = onlineCount;
        if (offlineEl) offlineEl.textContent = offlineCount;
        if (uptimeEl && d.servers.length) uptimeEl.textContent = fmtTime((d.servers[0] && d.servers[0].uptime) || 0);
    });
...
function serverTileAction(event, id, action, type) {
    ...
    post('/api/servers/select', { id: id }, function(result) {
        ...
        if (action === 'restart') restartServer(type);
        if (action === 'console') openConsole(type);
        setTimeout(loadServerOverview, 500);
    });
}
    ...
    <div class="server-grid">
        <div class="card"><div class="card-header"><h3 data-i18n="srv_world">${t('srv_world')}</h3></div><div class="card-body"><div class="control-panel">
            ...
            <div class="control-buttons">
                <button id="world-start" class="btn btn-success btn-lg" data-server-type="worldserver" onclick="startServer('worldserver')"><svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M8 5v14l11-7z"/></svg><span data-i18n="srv_start">${t('srv_start')}</span></button>
                <button id="world-stop" class="btn btn-danger btn-lg" data-server-type="worldserver" onclick="stopServer('worldserver')"><svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M6 6h12v12H6z"/></svg><span data-i18n="srv_stop">${t('srv_stop')}</span></button>
                <button id="world-restart" class="btn btn-warning btn-lg" data-server-type="worldserver" onclick="restartServer('worldserver')"><span>↻</span><span data-i18n="srv_restart">${t('srv_restart')}</span></button>
                <button id="world-console" class="btn btn-secondary btn-lg" data-server-type="worldserver" onclick="openConsole('worldserver')"><span>&gt;_</span><span data-i18n="con_popup">${t('con_popup')}</span></button>
            </div>
        ...
        <div class="card"><div class="card-header"><h3 data-i18n="srv_auth">${t('srv_auth')}</h3></div><div class="card-body"><div class="control-panel">
            ...
            <div class="control-buttons">
                <button id="bnet-start" class="btn btn-success btn-lg" data-server-type="bnetserver" onclick="startServer('bnetserver')"><svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M8 5v14l11-7z"/></svg><span data-i18n="srv_start">${t('srv_start')}</span></button>
                <button id="bnet-stop" class="btn btn-danger btn-lg" data-server-type="bnetserver" onclick="stopServer('bnetserver')"><svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M6 6h12v12H6z"/></svg><span data-i18n="srv_stop">${t('srv_stop')}</span></button>
                <button id="bnet-restart" class="btn btn-warning btn-lg" data-server-type="bnetserver" onclick="restartServer('bnetserver')"><svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z"/></svg><span data-i18n="srv_restart">${t('srv_restart')}</span></button>
                <button id="bnet-console" class="btn btn-secondary btn-lg" data-server-type="bnetserver" onclick="openConsole('bnetserver')"><span>&gt;_</span><span data-i18n="con_popup">${t('con_popup')}</span></button>
            </div>
...
function syncServerControls() {
    var statusMap = {};
    var w = document.getElementById('wStatus');
    var b = document.getElementById('bStatus');
    statusMap.worldserver = !!(w && w.classList.contains('online'));
    statusMap.bnetserver = !!(b && b.classList.contains('online'));

    ['worldserver', 'bnetserver'].forEach(function(type) {
        var startBtn = document.getElementById(type === 'worldserver' ? 'world-start' : 'bnet-start');
        var stopBtn = document.getElementById(type === 'worldserver' ? 'world-stop' : 'bnet-stop');
        var restartBtn = document.getElementById(type === 'worldserver' ? 'world-restart' : 'bnet-restart');
        var consoleBtn = document.getElementById(type === 'worldserver' ? 'world-console' : 'bnet-console');
        var running = !!statusMap[type];
        if (startBtn) { startBtn.disabled = running; startBtn.style.opacity = running ? '0.5' : '1'; startBtn.dataset.running = String(running); }
        if (stopBtn) { stopBtn.disabled = !running; stopBtn.style.opacity = running ? '1' : '0.5'; }
        if (restartBtn) { restartBtn.disabled = !running; restartBtn.style.opacity = running ? '1' : '0.5'; }
        if (consoleBtn) { consoleBtn.disabled = false; consoleBtn.style.opacity = '1'; }
    });
}

function updateStatus(s) {
    if (!s) return;
    var world = s.worldserver || { running: false, pid: null };
    var auth = s.bnetserver || { running: false, pid: null };
    var el;
    el = document.getElementById('wsStatus'); if (el) { el.className = 'status-badge ' + (world.running ? 'online' : 'offline'); el.textContent = t(world.running ? 'dash_online' : 'dash_offline'); }
    el = document.getElementById('wsPid'); if (el) el.textContent = world.pid || '-';
    el = document.getElementById('bsStatus'); if (el) { el.className = 'status-badge ' + (auth.running ? 'online' : 'offline'); el.textContent = t(auth.running ? 'dash_online' : 'dash_offline'); }
    el = document.getElementById('bsPid'); if (el) el.textContent = auth.pid || '-';
    el = document.getElementById('uptime'); if (el) el.textContent = fmtTime(s.uptime || 0);
    el = document.getElementById('wStatus'); if (el) { el.className = 'status-badge ' + (world.running ? 'online' : 'offline'); el.textContent = t(world.running ? 'dash_online' : 'dash_offline'); }
    el = document.getElementById('wPid'); if (el) el.textContent = world.pid || '-';
    el = document.getElementById('bStatus'); if (el) { el.className = 'status-badge ' + (auth.running ? 'online' : 'offline'); el.textContent = t(auth.running ? 'dash_online' : 'dash_offline'); }
    el = document.getElementById('bPid'); if (el) el.textContent = auth.pid || '-';
    if (s.system) {
        var sys = s.system;
        var cpu = Math.min(100, (sys.loadAverage && sys.cpuCores ? (sys.loadAverage[0] / sys.cpuCores) * 100 : 0));
        ...
        el = document.getElementById('memPct'); if (el) el.textContent = (sys.memoryUsagePercent || 0) + '%';
        el = document.getElementById('memBar'); if (el) el.style.width = (sys.memoryUsagePercent || 0) + '%';
        ...
        el = document.getElementById('loadVal'); if (el) el.textContent = (sys.loadAverage && sys.loadAverage[0] ? sys.loadAverage[0].toFixed(2) : '0.00');
        el = document.getElementById('loadBar'); if (el) el.style.width = Math.min(100, (sys.loadAverage && sys.cpuCores ? (sys.loadAverage[0] / sys.cpuCores) * 100 : 0)) + '%';
    }
    syncServerControls();
}
...
function selectServerProfile(id) {
    post('/api/servers/select', { id: id }, function(d) {
        if (!d.success) return;
        loadServerProfiles();
        get('/api/servers/status', function(statusRes) {
            if (statusRes && statusRes.success && statusRes.servers) {
                var current = statusRes.servers.find(function(server) { return server.id === id; }) || statusRes.servers[0];
                if (current) {
                    updateStatus({
                        worldserver: current.worldserver || { running: false, pid: null },
                        bnetserver: current.bnetserver || { running: false, pid: null },
                        uptime: 0,
                        system: null
                    });
                }
        ...
        if (currentPage === 'dashboard') loadServerOverview();
    });
}
...
/* ========== ACTIONS ========== */
function startServer(type) { post('/api/server/start', { type: type }, function() { setTimeout(loadServerOverview, 500); }); }
function stopServer(type) { if (confirm(type + '?')) post('/api/server/stop', { type: type }, function() { setTimeout(loadServerOverview, 500); }); }
function restartServer(type) { if (confirm(type + '?')) post('/api/server/restart', { type: type }, function() { setTimeout(loadServerOverview, 500); }); }
