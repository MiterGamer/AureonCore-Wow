        ...
        if (wdt.loadFile(CascStorage, map_ids[z].WdtFileDataId, Trinity::StringFormat("WDT for map {}", map_ids[z].Id), false))
        {
            ...
            for (uint32 y = 0; y < WDT_MAP_SIZE; ++y)
            {
                for (uint32 x = 0; x < WDT_MAP_SIZE; ++x)
                {
                    ...
                    {
                        uint32 rootADT = maid->As<wdt_MAID>()->adt_files[y][x].rootADT;
                        if (rootADT)
                        {
                            existingTiles[y * WDT_MAP_SIZE + x] = ConvertADT(rootADT, map_ids[z].Name, outputFileName, y, x, build, ignoreDeepWater);
                        }

                        if (!existingTiles[y * WDT_MAP_SIZE + x])
                        {
