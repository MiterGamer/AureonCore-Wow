...
static void StartConversationWithActors(uint32 conversationId, Player* player, ObjectGuid actor0, ObjectGuid actor1 = ObjectGuid::Empty)
{
    Conversation* conversation = Conversation::CreateConversation(conversationId, player, *player, player->GetGUID(), nullptr, false);
    if (!conversation)
        return;

    conversation->AddActor(0, 0, actor0);
    conversation->AddActor(0, 1, actor1.IsEmpty() ? player->GetGUID() : actor1);
...
public:
    ...
    void OnQuestStatusChange(Player* player, Quest const* quest, QuestStatus oldStatus, QuestStatus newStatus) override
    {
        ...
        if (newStatus == QUEST_STATUS_REWARDED)
        {
            ...
            if (!garrickUpperDeck)
                garrickUpperDeck = FindCreatureIgnorePhase(player, "q58208_garrick", 75.0f);
...
public:
    ...
    void OnQuestStatusChange(Player* player, Quest const* quest, QuestStatus oldStatus, QuestStatus newStatus) override
    {
        ...
        if (newStatus == QUEST_STATUS_REWARDED)
        {
            ...
            if (!grimaxeUpperDeck)
                grimaxeUpperDeck = FindCreatureIgnorePhase(player, "q59928_grimaxe", 75.0f);
...
class spell_summon_sparring_partner : public SpellScript
{
    ...
    void SelectTarget(WorldObject*& target)
    {
        ...
        if (!partner)
            partner = FindCreatureIgnorePhase(caster, caster->GetTeam() == ALLIANCE ? "q58208_cole" : "q59928_throg", 10.0f);
        if (!partner)
            return;
...
struct npc_sparring_partner_exiles_reach : public ScriptedAI
{
    ...
    void StartPrivateConversation(uint32 conversationId)
    {
        if (Unit* owner = me->GetDemonCreator())
        {
            ...
            if (!conversation)
                return;
            conversation->AddActor(_actorId, 0, me->GetGUID());
            conversation->AddActor(_actorId, 1, me->GetGUID());
            conversation->Start();
        }
    }
...
enum FirstMateStandYourGroundData
{
    ...
    SPELL_SUMMON_COLE_COMBAT            = 303065,
    SPELL_SUMMON_THROG_COMBAT           = 325108,
};
...
struct npc_first_mate_stand_your_ground : public ScriptedAI
{
    ...
    {
        if (quest->GetQuestId() != QUEST_STAND_YOUR_GROUND_ALLIANCE && quest->GetQuestId() != QUEST_STAND_YOUR_GROUND_HORDE)
            return;
...
static void TeleportToExilesReachBeach(Player* player, bool alliance);

static void SetWorldCaptainVisible(Player* player, bool visible)
{
    char const* ids[2] = {};
    if (player->GetTeam() == ALLIANCE)
    {
        ids[0] = "q56775_garrick_lower_deck";
        ids[1] = "q58208_garrick";
    }
    else
    {
        ids[0] = "q59926_grimaxe_lower_deck";
        ids[1] = "q59928_grimaxe";
    }

    for (char const* stringId : ids)
    {
        if (!stringId)
            continue;
        if (Creature* captain = FindCreatureIgnorePhase(player, stringId, 80.0f))
        {
            if (captain->IsPrivateObject() || captain->IsSummon())
                continue;
            captain->SetVisible(visible);
            captain->SetUninteractible(!visible);
        }
    }
}
...
public:
    ...
    void OnQuestStatusChange(Player* player, Quest const* quest, QuestStatus /*oldStatus*/, QuestStatus newStatus) override
    {
        if (newStatus == QUEST_STATUS_INCOMPLETE)
        {
            ...
            SetWorldCaptainVisible(player, false);
        ...
        if (newStatus == QUEST_STATUS_NONE)
            SetWorldCaptainVisible(player, true);

        if (newStatus != QUEST_STATUS_REWARDED)
            return;

        // RewardSpell 305425/325131 is supposed to play the crash movie and
        // teleport. On this client the dest often does not apply, so force it.
        if (quest->GetQuestId() == QUEST_BRACE_FOR_IMPACT_ALLIANCE)
        {
            player->SendMovieStart(895);
            player->CastSpell(player, 305446, true);
            TeleportToExilesReachBeach(player, true);
        }
        else if (quest->GetQuestId() == QUEST_BRACE_FOR_IMPACT_HORDE)
        {
            player->SendMovieStart(931);
            player->CastSpell(player, 325133, true);
            TeleportToExilesReachBeach(player, false);
        }
    }
    ...
    void SpawnActors(Player* player, TeamId team, Position petSpawnPos)
    {
        ...
        {
            Creature* source = FindCreatureIgnorePhase(player, actor.StringId, 50.0f);
            if (!source)
            {
                if (actor.StringId == "q58208_cole")
                    source = FindCreatureIgnorePhase(player, "q58209_cole", 50.0f);
                else if (actor.StringId == "q59928_throg")
                    source = FindCreatureIgnorePhase(player, "q59927_throg", 50.0f);
            }
            SpawnActor(player, source, actor.ActorPosition);
        }
    ...
    void SpawnActor(Player* player, Creature* creature, Position const& position)
    {
        ...
        // Ship is map 2261/2369 here, not a continent transport (log: Spawned 0 transports).
...
{
    npc_ship_captain_brace_for_impact_private(Creature* creature) : ScriptedAI(creature), _pathPreTalk(0), _pathPostTalk(0), _waitTime(0s), _arrived(false) { }
    ...
    bool OnGossipHello(Player* player) override
    {
        ...
        uint32 const questId = me->GetEntry() == NPC_CAPTAIN_GARRICK ? QUEST_BRACE_FOR_IMPACT_ALLIANCE : QUEST_BRACE_FOR_IMPACT_HORDE;
...
static Position const AllianceCrashBeachPos = { -446.1f, -2607.1f, 0.48f, 0.103f };
static Position const HordeCrashBeachPos    = { -462.4f, -2619.8f, 0.40f, 0.103f };

static void TeleportToExilesReachBeach(Player* player, bool alliance)
{
    Position const& dest = alliance ? AllianceCrashBeachPos : HordeCrashBeachPos;
    player->TeleportTo(MAP_EXILES_REACH, dest.GetPositionX(), dest.GetPositionY(), dest.GetPositionZ(), dest.GetOrientation());
}

static bool IsStrandedAfterShipCrash(Player const* player)
{
    uint32 const mapId = player->GetMapId();
    if (mapId == 2261 || mapId == 2369)
        return true;

    // RewardSpell 305425 sometimes drops the player at the transport world
    // position on 2175 instead of the beach dest (-462, -2619).
    if (mapId == MAP_EXILES_REACH && player->GetPositionX() > 500.0f)
        return true;
...
public:
    ...
    void OnMovieComplete(Player* player, uint32 movieId) override
    {
        switch (movieId)
        {
            case MOVIE_ALLIANCE_SHIP_CRASH:
                ...
                if (IsStrandedAfterShipCrash(player))
                    TeleportToExilesReachBeach(player, true);
                break;
    ...
    {
        if (player->GetQuestRewardStatus(QUEST_BRACE_FOR_IMPACT_ALLIANCE) &&
            player->GetQuestStatus(55122) == QUEST_STATUS_NONE &&
            IsStrandedAfterShipCrash(player))
            TeleportToExilesReachBeach(player, true);
        else if (player->GetQuestRewardStatus(QUEST_BRACE_FOR_IMPACT_HORDE) &&
            player->GetQuestStatus(59929) == QUEST_STATUS_NONE &&
            IsStrandedAfterShipCrash(player))
            TeleportToExilesReachBeach(player, false);
    }
};
    ...
    void OnQuestAccept(Player* player, Quest const* quest) override
    {
        switch (quest->GetQuestId())
        {
            case QUEST_MURLOC_MANIA_ALLIANCE:
                StartConversationWithActors(CONVERSATION_QUEST_MURLOC_MANIA_ALLIANCE, player, me->GetGUID());
                break;
    ...
    void OnQuestAccept(Player* player, Quest const* quest) override
    {
        switch (quest->GetQuestId())
        {
            case QUEST_MURLOC_MANIA_HORDE:
                StartConversationWithActors(CONVERSATION_QUEST_MURLOC_MANIA_HORDE, player, me->GetGUID());
                break;
...
struct npc_alliance_survivors_beach_laying : public ScriptedAI
{
    ...
    void SpellHit(WorldObject* caster, SpellInfo const* spellInfo) override
    {
        ...
        if (Player* player = caster->ToPlayer())
        {
            ...
            StartConversationWithActors(ConversationId, player, me->GetGUID());
            player->UpdateVisibilityForPlayer(); // spawn tracking swaps laying -> standing
        }
    }
};
