...
class spell_mage_flame_on : public AuraScript
{
   bool Validate(SpellInfo const* spellInfo) override
   {
       // Spell 205029 does not have EFFECT_2 in this DBC build - skip effect validation
   ...
   {
       canBeRecalculated = false;
       // EFFECT_2 may not exist in this DBC build - use safe default
       if (GetSpellInfo()->GetEffects().size() > EFFECT_2)
...
class spell_mage_prismatic_barrier : public AuraScript
{
    bool Validate(SpellInfo const* spellInfo) override
    {
        // Spell 235450 does not have EFFECT_5 in this DBC build - skip effect validation
        return true;
    }
    ...
    {
        canBeRecalculated = false;
        // EFFECT_5 may not exist in this DBC build - use safe default
        if (Unit* caster = GetCaster())
        {
            if (GetSpellInfo()->GetEffects().size() > EFFECT_5)
