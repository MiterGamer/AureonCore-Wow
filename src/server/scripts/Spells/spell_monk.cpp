...
class spell_monk_crackling_jade_lightning : public AuraScript
{
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        // SPELL_MONK_STANCE_OF_THE_SPIRITED_CRANE (154436) does not exist in this DBC build
        return ValidateSpellInfo({ SPELL_MONK_CRACKLING_JADE_LIGHTNING_CHI_PROC });
    }
...
class spell_monk_save_them_all : public AuraScript
{
    bool Validate(SpellInfo const* spellInfo) override
    {
        // Spell 389579 does not have EFFECT_2 in this DBC build - skip effect validation
        return ValidateSpellInfo({ SPELL_MONK_SAVE_THEM_ALL_HEAL_BONUS });
    }

    bool CheckProc(ProcEventInfo const& eventInfo) const
    {
        // EFFECT_2 may not exist in this DBC build - default to allowing proc
