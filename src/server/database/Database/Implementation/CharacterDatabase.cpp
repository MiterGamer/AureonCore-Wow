...
#define CharacterSelectEquipment(table_alias) \
    ...
    PrepareStatement(CHAR_SEL_WARBAND_GROUPS, "SELECT groupId, orderIndex, warbandSceneId, flags, contentSetId, name FROM character_warband_groups WHERE battlenetAccountId = ? ORDER BY orderIndex, groupId", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_WARBAND_GROUP_MEMBERS, "SELECT m.groupId, m.guid, m.warbandScenePlacementId, m.memberType, m.contentSetId FROM character_warband_group_members m INNER JOIN character_warband_groups g ON g.groupId = m.groupId WHERE g.battlenetAccountId = ? ORDER BY m.groupId, m.memberIndex", CONNECTION_ASYNC);
}
