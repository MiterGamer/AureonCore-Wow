...
void WorldSession::HandleOpenTradeskillNpc(WorldPackets::NPC::OpenTradeskillNpc& packet)
{
    ...
    auto openProfessions = [this, player](ObjectGuid const& guid)
    {
        player->PlayerTalkClass->GetInteractionData().StartInteraction(guid, PlayerInteractionType::Professions);

        WorldPackets::NPC::NPCInteractionOpenResult result;
        result.Npc = guid;
        result.InteractionType = PlayerInteractionType::Professions;
        result.Success = true;
        SendPacket(result.Write());
    };

    if (Creature* npc = player->GetNPCIfCanInteractWith(packet.Npc,
        UNIT_NPC_FLAG_TRAINER | UNIT_NPC_FLAG_TRAINER_PROFESSION, UNIT_NPC_FLAG_2_TRADESKILL_NPC))
    {
        if (uint32 trainerId = npc->GetTrainerId())
        {
            SendTrainerList(npc, trainerId);
        ...
        openProfessions(npc->GetGUID());
        return;
    }

    if (Creature* npc = player->GetNPCIfCanInteractWith(packet.Npc, UNIT_NPC_FLAG_NONE, UNIT_NPC_FLAG_2_NONE))
    {
        openProfessions(npc->GetGUID());
        return;
    }

    if (GameObject* go = player->GetGameObjectIfCanInteractWith(packet.Npc))
        openProfessions(go->GetGUID());
}
