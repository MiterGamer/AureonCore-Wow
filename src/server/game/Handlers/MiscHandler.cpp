...
void WorldSession::HandleRequestMythicPlusSeasonData(WorldPackets::Null& /*packet*/)
{
    sSeasonalContentMgr->SendMythicPlusSeasonData(this);
}

void WorldSession::HandleRequestMythicPlusAffixes(WorldPackets::Null& /*packet*/)
{
    sSeasonalContentMgr->SendMythicPlusAffixes(this);
}

void WorldSession::HandleRequestWeeklyRewards(WorldPackets::Null& /*packet*/)
{
    sSeasonalContentMgr->SendWeeklyRewards(this);
}

void WorldSession::HandleGetCharacterCurrencyTransferLog(WorldPackets::Null& /*packet*/)
{
    sSeasonalContentMgr->SendCurrencyTransferLog(this);
}

void WorldSession::HandleGetLandingPageShipments(WorldPackets::Null& /*packet*/)
{
    sSeasonalContentMgr->SendLandingPageShipments(this);
}

void WorldSession::HandleRequestScheduledPvpInfo(WorldPackets::Null& /*packet*/)
{
    sSeasonalContentMgr->SendScheduledPvpInfo(this);
}

void WorldSession::HandlePerksProgramStatusRequest(WorldPackets::Null& /*packet*/)
{
    sSeasonalContentMgr->SendPerksProgramStatus(this);
}

void WorldSession::HandlePerksProgramRequestPurchase(WorldPackets::Misc::PerksProgramRequestPurchase& packet)
{
    sSeasonalContentMgr->HandlePerksPurchase(GetPlayer(), packet.VendorItemID);
}

void WorldSession::HandleStartChallengeMode(WorldPackets::Misc::StartChallengeMode& packet)
{
    sChallengeModeMgr->Start(GetPlayer(), packet.Item);
}

void WorldSession::HandleResetChallengeMode(WorldPackets::Null& /*packet*/)
{
    sChallengeModeMgr->Reset(GetPlayer());
}

void WorldSession::HandleClaimWeeklyReward(WorldPackets::Misc::ClaimWeeklyReward& packet)
{
    sSeasonalContentMgr->ClaimWeeklyReward(GetPlayer(), packet.Type, packet.Threshold);
}

void WorldSession::HandleLfgListGetStatus(WorldPackets::Null& /*packet*/)
{
    WorldPackets::LFG::LFGListSearchStatus status;
    ...
    lfg::LfgState state = sLFGMgr->GetState(GetPlayer()->GetGUID());
    status.Status = uint8(state);
    SendPacket(status.Write());
}

void WorldSession::HandleRequestLfgListBlacklist(WorldPackets::Null& /*packet*/)
{
    WorldPackets::LFG::LFGListUpdateBlacklist blacklist;
    blacklist.BlackList.PlayerGuid = GetPlayer()->GetGUID();

    lfg::LfgLockMap const locks = sLFGMgr->GetLockedDungeons(GetPlayer()->GetGUID());
    blacklist.BlackList.Slot.reserve(locks.size());
    for (auto const& [dungeonId, lock] : locks)
        blacklist.BlackList.Slot.emplace_back(dungeonId, lock.lockStatus, lock.requiredItemLevel, 0, 0);
