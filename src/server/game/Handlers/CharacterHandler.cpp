...
public:
    ...
    bool Initialize(uint32 accountId, uint32 battlenetAccountId, bool withDeclinedNames, bool isDeletedCharacters)
    {
        ...
        if (!isDeletedCharacters && battlenetAccountId)
        {
            ...
            stmt->setUInt32(0, battlenetAccountId);
            result &= SetPreparedQuery(WARBAND_MEMBERS, stmt);
        }
...
void WorldSession::HandleCharEnum(CharacterDatabaseQueryHolder const& holder)
{
    ...
    if (!charEnum.IsDeletedCharacters)
    {
        std::unordered_map<uint64, std::size_t> groupIndexById;
        if (PreparedQueryResult groups = holder.GetPreparedResult(EnumCharactersQueryHolder::WARBAND_GROUPS))
        {
            do
            {
                Field* fields = groups->Fetch();
                ...
                groupIndexById[group.GroupID] = charEnum.WarbandGroups.size() - 1;
            } while (groups->NextRow());
        }

        if (PreparedQueryResult members = holder.GetPreparedResult(EnumCharactersQueryHolder::WARBAND_MEMBERS))
        {
            do
            {
                Field* fields = members->Fetch();
                auto itr = groupIndexById.find(fields[0].GetUInt64());
                if (itr == groupIndexById.end())
                    continue;

                WorldPackets::Character::WarbandGroupMember& member = charEnum.WarbandGroups[itr->second].Members.emplace_back();
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder const& holder)
{
    ...
    // Send PVPSeason
    {
        ...
        seasonInfo.MythicPlusDisplaySeasonID = sSeasonalContentMgr->GetCurrentMythicPlusSeasonID();
        seasonInfo.MythicPlusMilestoneSeasonID = sSeasonalContentMgr->GetCurrentMythicPlusMilestoneSeasonID();
        seasonInfo.WeeklyRewardChestsEnabled = true;
