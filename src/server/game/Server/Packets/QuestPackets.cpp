...
{
    // Empty result: retail payload is large; keep this tiny so the client does not drop the session.
    _worldPacket << uint32(0); // item count
    _worldPacket << uint32(0); // extra count
    return &_worldPacket;
}
}
