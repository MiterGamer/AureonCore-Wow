...
{
    _worldPacket << uint32(0); // no voice proxies on this realm
    return &_worldPacket;
}
...
{
    // Count first so an unknown 12.1 layout cannot walk a huge reward array.
    _worldPacket << uint32(0);
    _worldPacket << int32(SeasonID);
    _worldPacket << uint32(NextReset);
    _worldPacket << uint32(Rewards.size());
    for (WeeklyRewardEntry const& reward : Rewards)
    {
        _worldPacket << int32(reward.Type);
        _worldPacket << int32(reward.Threshold);
        _worldPacket << int32(reward.Progress);
        _worldPacket << int32(reward.ActivityID);
        _worldPacket << int32(reward.RewardItemID);
    }
    return &_worldPacket;
}
...
WorldPacket const* LandingPageShipmentsResponse::Write()
{
    _worldPacket << uint32(0); // container / follower type
    _worldPacket << uint32(0); // no work-order payload until 12.1 layout is confirmed
    return &_worldPacket;
}

WorldPacket const* ScheduledPvpInfoResponse::Write()
{
    _worldPacket << int32(BrawlID);
...
WorldPacket const* PerksProgramVendorUpdate::Write()
{
    ...
    for (WorldPackets::PerksProgram::PerksVendorItem const& item : Items)
        _worldPacket << item;
    return &_worldPacket;
}
...
WorldPacket const* ChallengeModeStart::Write()
{
    ...
    _worldPacket << int32(ChallengeModeID);
    _worldPacket << int32(KeystoneLevel);
    for (int32 affix : Affixes)
        _worldPacket << int32(affix);
    _worldPacket << uint32(DeathCount);
    _worldPacket << uint32(TimeLimitMS);
    return &_worldPacket;
}
...
{
    _worldPacket << int32(ChallengeModeID);
    _worldPacket << int32(DurationMS);
    _worldPacket << int32(KeystoneLevel);
    _worldPacket << Bits<1>(Timed);
    _worldPacket.FlushBits();
    return &_worldPacket;
}
