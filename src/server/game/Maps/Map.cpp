...
void InstanceMap::UpdateInstanceLock(UpdateBossStateSaveDataEvent const& updateSaveDataEvent)
{
    if (i_instanceLock)
    {
        ...
        for (MapReference& mapReference : m_mapRefManager)
        {
            ...
            if (updateSaveDataEvent.NewState == DONE)
            {
                sSeasonalContentMgr->OnEncounterComplete(player, this, updateSaveDataEvent.DungeonEncounter);
                sChallengeModeMgr->OnEncounterComplete(player, this, updateSaveDataEvent.DungeonEncounter);
            }
        }
