...
class ServerRuntime {
  ...
  async start(type) {
    ...
    try {
      ...
      child.onExit(({ exitCode }) => { state.exited = true; this.emit(type, 'warn', `=== Ended (${exitCode}) ===`); });
  ...
  startRemote(type, profile, key) {
    ...
    return new Promise(resolve => {
      ...
      connection.on('ready', () => connection.shell({ term: 'xterm-256color', cols: 160, rows: 40 }, (error, stream) => {
        ...
        stream.on('close', () => { state.exited = true; connection.end(); this.emit(type, 'warn', `=== ${type} ended ===`); });
  ...
  stop(type) {
    ...
    if (state.remote) { state.process.write('\u0003'); state.process.end(); } else state.process.kill();
    return { success: true };
  }
